...
# st.header('st.button') ## This a header text for the app 

# if st.button("Say hello"): ## the button method accpets the label input argument of 'say hello' 
#     st.write("Why hello there")
# else: ## use an if else to handle different messages
#     st.write("Goodbye") ## this is the default message
...
# add_sidebar = st.sidebar.selectbox("Aggregate or Individual Video", ('Aggregate Metrics', 'Individual Video Analysis')) ## Adding a Sidebar

# df_agg_diff = df_agg.copy()
# metric_date_12mo = df_agg_diff['Video publish time'].max() - pd.DateOffset(month=12)
# median_agg = df_agg_diff[df_agg_diff['Video publish time'] >= metric_date_12mo].median()
# numeric_cols = np.array((df_agg_diff.dtypes == 'float65') | (df_agg_diff.dtypes == 'int64'))
...
#     df_agg_metrics = df_agg[['Views','Likes','Subscribers','Shares','Comments added','RPM(USD)','Average % viewed',
#                              'Avg_duration_sec', 'Engagement_ratio','Views / sub gained']] # choose the metrics that you want
...
#     metric_medians6mo = df_agg_metrics[df_agg['Video publish time'] >= metric_date_6mo].median()
#     metric_medians12mo = df_agg_metrics[df_agg['Video publish time'] >= metric_date_12mo].median()
    
#     #st.metric('Views', metric_medians6mo['Views'], 500) # label, value, other information
...
#             count += 1
#             if count >= 5: # wrapping function
#                 count = 0

#     df_agg_diff['Publish_date'] = df_agg_diff['Video publish time'].apply(lambda x: x.date())
#     df_agg_diff_final = df_agg_diff.loc[:, ['Views', 'Likes', 'Subscribers', 'Shares', 
#                                         'Comments added', 'RPM(USD)', 'Average % viewed',
#                                         'Avg_duration_sec', 'Engagement_ratio', 
#                                         'Views / sub gained']]
...
#     video_select = st.selectbox("Pick a Video:", videos) # Label, Options 
...
# Example 1: 
st.write("Hello, *World!* :sunglasses:") # markdown formats
# Example 2: 
st.write(1234)
# Example 3: 
df = pd.DataFrame({
...
c = alt.Chart(df2).mark_circle().encode( # another way of displaying graphs 
    x = 'a', y = 'b', size= 'c', tooltip= ['a', 'b', 'c']
)
st.write(c)

# # Other methods of displaying text: 
# # st.markdown
# # st.header
# # st.subheader
# # st.caption
